...
class unzipper: 
    def unzip(filename):  
        ...
        kml = kmz.open('overlay.kml', 'r').read()
        os.system('cp %s unzipfile.zip' %filename)
        os.system('unzip unzipfile.zip')
        return kml
class get_columns:
...
class kmz_to_csv_class:
    ...
    def kmz_to_csv(rootDir, kmzFile, csvFile, company_name):
        """
        Open the KMZ. Read the KML. Open a CSV file. Process a coordinate string to be a CSV row.
        """
        with open(rootDir + kmzFile, encoding='utf8') as f:
            ...
            with open(rootDir + csvFile, 'w', newline='', encoding='utf8') as csvfile:
                ...
                for placemark in s.find_all('Placemark'):
                    ...
                    #print(namelower)
                    #print(nameupper)
                    if namelower in kmz_to_csv_class.water or nameupper in kmz_to_csv_class.water:
                        types = "water"
                    elif namelower in kmz_to_csv_class.non_water or nameupper in kmz_to_csv_class.non_water:
                        types = "nonwater"
                    else:
                        types = "not used"
...
def main():  
    ...
    filename = input("Please insert [[KMZ]] File name with file type! ex) 2019_06_08.kmz  :")
    unzipper.unzip(filename)
    
    csvName = filename
    csvName = csvName.replace('.kmz','.csv')
    print(csvName)

    kmz_to_csv_class.kmz_to_csv('', 'overlay.kml', csvName,'')
